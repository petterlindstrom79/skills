...
def resolve_image(body: dict) -> str:
    if body.get('image_url'):
        return body['image_url']
    if body.get('image_base64'):
        return f"data:image/png;base64,{body['image_base64']}"
    ...
    raise SystemExit(json.dumps({'ok': False, 'error': 'one_of_image_url_image_path_image_base64_required'}, ensure_ascii=False))


def main():
    if len(sys.argv) < 2:
        raise SystemExit('Usage: img2img.py \'{"prompt":"...","image_path":"..."}\'')
