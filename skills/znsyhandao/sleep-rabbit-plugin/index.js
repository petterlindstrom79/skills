...
async handleSleepAnalysis(params, context) {
  try {
    ...
    if (!fs.existsSync(edf_file)) {
      return { error: true, message: `EDF文件不存在: ${edf_file}` };
    }
    
    // ✅ 安全地传递参数（不拼接）
    const result = await this.runPythonScript('sleep', [
      edf_file,  // 直接传递路径，作为独立参数
      '--format', 'json'
    ]);
    ...
    return { error: true, message: `睡眠分析失败: ${error.message}` };
  }
}
...
async runPythonScript(command, args) {
  try {
    // 使用 execFile 或 spawn，避免shell注入
    const { spawn } = require('child_process');
    
    return new Promise((resolve, reject) => {
      // 构建参数数组 - 安全传递，不经过shell解析
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          reject(new Error(`Python进程退出码 ${code}: ${stderr}`));
        } else {
          resolve(stdout);
        }
      });
