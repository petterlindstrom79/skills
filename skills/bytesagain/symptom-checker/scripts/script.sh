#!/usr/bin/env bash
# symptom-checker - Health and wellness tracker
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${SYMPTOM_CHECKER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/symptom-checker}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
symptom-checker v$VERSION

Health and wellness tracker

Usage: symptom-checker <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "symptom-checker v$VERSION" ;;
