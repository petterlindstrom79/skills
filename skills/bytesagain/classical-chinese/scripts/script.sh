#!/usr/bin/env bash
# classical-chinese - Chinese content creation tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CLASSICAL_CHINESE_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/classical-chinese}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
classical-chinese v$VERSION

Chinese content creation tool

Usage: classical-chinese <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "classical-chinese v$VERSION" ;;
